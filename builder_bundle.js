// TNR content builder bundle v4.15 - loaded via @require by the tiny VM loader.
...
const PHR={jutsu:0,asset:2,item:3,ai:4,aiProfile:5,quest:6};const phr=r=>r.entity==='jutsu'?(r.slot==='create'?0:1):(PHR[r.entity]!==undefined?PHR[r.entity]:9);
const collectRefs=(o,out)=>{if(Array.isArray(o))o.forEach(x=>collectRefs(x,out));else if(o&&typeof o==='object'){for(const k in o)collectRefs(o[k],out)}else if(typeof o==='string'){const m=o.match(/^@(jutsu|ai|scene|item|quest):(.+)$/);if(m)out.push({pfx:m[1],key:m[2]})}};
...
const LCORE={damage:{elements:1},pierce:{elements:1},wound:{},increasedamagegiven:{},decreasedamagetaken:{},increasedamagetaken:{},increasestat:{},decreasestat:{},absorb:{},reflect:{},shield:{},heal:{poolsAffected:1},stun:{apReduction:1},seal:{},moveprevent:{},drain:{poolsAffected:1},decreasehealgiven:{},increasecooldown:{},clear:{},cleanse:{},debuffprevent:{},copy:{},lifesteal:{}};
...
for(const f of(Array.isArray(d.effects)?d.effects:[])){if(!f||!f.type)continue;
 if(LFORM[f.type]&&(!Array.isArray(f.statTypes)||!f.statTypes.length||!Array.isArray(f.generalTypes)||!f.generalTypes.length))E('L06 '+f.type+' missing statTypes/generalTypes');
 if('direction'in f&&f.direction!=='offence')E('L07 '+f.type+' direction "'+f.direction+'" (must be "offence")');
...
const _un=(JSON.stringify(r.data)||'').match(/@(jutsu|ai|scene|item|quest):[^"]{1,60}/);if(_un){sr(r,'error','unresolved ref '+_un[0]);return}
...
try{const bundle={builder:'v4.15',at:now(),entries:rows.map(r=>({name:r.name,srcId:r.key||null,entity:r.entity,slot:r.slot,state:r.state,detail:r.detail||'',id:r.outId||idmap[r.key]||r.targetId||null,pushed:r.pushed||null})),idmap};dl('tnr_results_'+Date.now()+'.json',JSON.stringify(bundle,null,1))}catch(_e){}
...
R.innerHTML='<style>'+CSS+'</style><button class="k-fab">▶ Build</button><div class="k-pn"><div class="k-hd"><span class="k-ti">Content builder v4.15</span><button class="k-ic k-min">▾</button></div><div class="k-bw"><div class="k-bar"></div></div><div class="k-bd"><textarea class="k-in" placeholder="paste a manifest or 📄 Load a JSON file"></textarea><div class="k-st2">paste a manifest to preview</div><div class="k-ls"></div></div><div class="k-ft"><button class="k-bt k-go">▶ Build</button><button class="k-bt k-lm">📄 Load</button><button class="k-bt k-im">🖼 Imgs</button><button class="k-bt k-rt">↻ Retry failed</button><button class="k-bt k-cp">⧉ Copy</button><button class="k-bt k-me">🗺 Map</button><button class="k-bt k-mi">⤒ Map</button><button class="k-bt k-rs">⌫ Reset</button><input class="k-fi" type="file" multiple hidden><input class="k-mf" type="file" accept=".json,application/json" hidden><input class="k-mi2" type="file" accept=".json,application/json" hidden></div></div>';
