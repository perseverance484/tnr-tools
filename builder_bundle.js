...
//        silently running the jutsu fallback (push/16 misroute: 'gameAsset' created 9 junk jutsu).
...
// integrity ledger. Read-only, never writes. Uses the panel PAT when stored, anonymous
// otherwise (repo is public). Numbered prompt picker - no new UI surface, Android-safe.
...
const ghRHdr=()=>{const g=ghGet();const h={'accept':'application/vnd.github+json'};if(g.pat)h['authorization']='Bearer '+g.pat;return h};
const ghList=async dir=>{const r=await fetch('https://api.github.com/repos/'+GH.owner+'/'+GH.repo+'/contents/'+dir+'?ref='+GH.branch,{headers:ghRHdr()});
 if(!r.ok)throw new Error('list '+(dir||'(root)')+': HTTP '+r.status);const j=await r.json();
 if(!Array.isArray(j))throw new Error((dir||'(root)')+' is not a directory');return j};
const ghFetchRaw=async path=>{const h=ghRHdr();h['accept']='application/vnd.github.raw+json';
 const r=await fetch('https://api.github.com/repos/'+GH.owner+'/'+GH.repo+'/contents/'+path+'?ref='+GH.branch,{headers:h});
 if(!r.ok)throw new Error('fetch '+path+': HTTP '+r.status);return await r.arrayBuffer()};
...
if(r.entity&&r.entity!=='jutsu'){sr(r,'error','unknown entity "'+r.entity+'" - no route, entry blocked (v4.31 guard; push/16 misroute)');return}
