...
**Quest Studio foundation status — reviewed and accepted.** The ChatGPT-owned slice was read by this package at `824c4d58` (Quest Source, a request branch, an Actions worker, `quest_compile.py`, a Mission adapter, and a Forge seam). It has since been independently reviewed and **accepted for integration planning** at `chatgpt/forge-quest-studio-foundation@5ba636d85fe2dbd4e4bf0e2baa6070cdd7321b15` (review: `claude/quest-studio-final-review@82bb686370f3f8cbbe57068fe42b887c90fdf48d`, verdict APPROVE_WITH_NONBLOCKING_FOLLOWUP). Two things changed materially after the `824c4d58` reading and are authoritative here: the trigger is **source-push**, not workflow dispatch (`RUL-2026-09-16-002`), and the browser credential is **Contents: write only — no Actions write, no Workflows write**. This package still neither patches nor re-plans the slice; under the one-writer rule, findings about it go to its owner.
...
|---|---|---|
| 0 Foundations | Green `main`, one CI job, the measurement harness, domain logic below the seam with the UI byte-identical | none; its three entries are engineering positions |
...
**Explicitly not in the roadmap:** any TheNinjaRPG source change; an AI assistance layer in the Studio; Guide Studio and the infographic lane as production lanes; the guide content class until the pin moves; canonical-owner changes such as structured blocker codes in `mission.py` or profile-shape enforcement, which need their own Lane A briefs; re-implementing or patching anything on the ChatGPT Studio branch; re-authoring that branch's unrelated release-loader assertion changes; Studio v1.5 and v2 scope; any deletion affordance; and widening the full-capture allowlist or the 512 KiB ceiling as a side effect of a phase. `G_ROADMAP.md` §G.4 states where each lives instead.

---

## 5. Decisions the director must make before implementation can start

By id only, and only entries the director owns. `K_USER_DECISIONS.md` is the owner of every entry, with the options, the practical consequence and a recommendation where one is justified; §K.1 there separates director decisions from Fable's engineering decisions, so that this list is what actually waits on a ruling. Nothing here is settled.

**Blocks the first brief (phase 0): nothing the director owns.** K-13 (source-pin refresh), K-14 (minified bundle) and K-60 (the release-loader baseline fix) are engineering decisions under `K_USER_DECISIONS.md` §K.1, and the phase-0 brief fixes and defends them. What phase 0 waits on is objective: a green or explicitly isolated `main`, a re-run source-drift check, a named owner and branch, and a written bundle and release approach. The first director rulings the roadmap actually needs are phase 1's (K-06, K-17 director halves) and phase 2's shell decisions.

**Blocks the second brief (phase 1), director halves only:** **K-06** what classes of captured data may persist to the public repository; **K-17** which non-content procedures Forge may read at all. The tier mechanism and the per-row audit under them are engineering.
...
Fifty-nine entries are registered, K-01 to K-61 with `K-18` and `K-19` left as unused numbering gaps. Each carries an ownership class (§K.1): thirty-nine are director decisions, five are split, and fifteen are Fable's own engineering choices, which carry Fable's position rather than a question and wait on nobody. `K_USER_DECISIONS.md` §K.0 records the three rulings already made during the pass and what stays open under each.
...
**Acting on it.** A Lane A implementation brief for phase 0 and nothing else, committed as `state/prompt_<task>.md` in the existing pattern, pinning the ten headings `G_ROADMAP.md` §G.5 lists: base SHA and branch with one named owner, the release-loader integration plan, the defended positions on K-13 and K-14, the exact file list below the seam that may be touched, the DOM fixture set and its byte-identical acceptance, the CI shape, the bundle budget numbers, the storage promise, the socket-free requirement, and what is out of scope. Phase 0 needs no director ruling first; the rulings in section 5 are needed before phases 1 and 2 are briefed, and each is recorded in `docs/RULINGS.md` with its operative rule moved to its canonical owner. A phase does not start without a brief and a director handoff. Independent review of this package is not, by itself, approval of the roadmap; the director approves the roadmap separately.
