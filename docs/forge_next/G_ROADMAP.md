...
**Studio phase preconditions, stated once.** No Studio-phase work starts before all four hold: **(a)** the Quest Studio foundation has been independently reviewed **and integrated into `main`** — the review is **done**: the implementation was accepted at `chatgpt/forge-quest-studio-foundation@5ba636d85fe2dbd4e4bf0e2baa6070cdd7321b15` (`claude/quest-studio-final-review@82bb686370f3f8cbbe57068fe42b887c90fdf48d`, APPROVE_WITH_NONBLOCKING_FOLLOWUP), so what remains of (a) is integration, which carries its own gate: the branch is behind `main`, both `RUL-2026-09-16-001` and `RUL-2026-09-16-002` must survive the reconciliation, and one repository-only end-to-end `studio/quest/*` rehearsal must run after the seam reaches `main` with zero live-game contact; **(b)** the director half of **K-26** is ruled; **(c)** the director half of **K-15** is ruled; **(d)** every `824c4d58:` cite in this package has been re-resolved at the accepted Studio SHA `5ba636d85fe2dbd4e4bf0e2baa6070cdd7321b15`, with the three paths that changed between the snapshot and `cda8ac76` re-read in full, plus `.github/workflows/quest_studio.yml`, `docs/RULINGS.md` and `docs/design/FORGE_NEXT_REPO_BACKED_STUDIO_ARCHITECTURE.md`, all of which changed again between `cda8ac76` and the accepted SHA (`00_CONTEXT.md` §00.6 measures the first hop; the second hop is the source-push correction). Until then nothing on that branch is assumed to exist here, nothing on it is re-implemented or patched from a Fable branch, and the package cites it by SHA only (RM-01, RM-03; `CLAUDE.md` §4).
