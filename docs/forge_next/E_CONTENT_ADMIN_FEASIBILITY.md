...
4. **CA row verdict tally as merged** (25 content-class rows): zero-game-change `YES` on 21 rows, `PARTIAL` on 3 (CA-04 gameAsset bytes, CA-12 sectorMap, CA-21 jutsu reskin moderation), `YES but excluded by ownership` on CA-25 and CA-17 (balance knobs and ranked-season rewards, both user-owned; see K-53), and `YES on the procedures but forbidden by engine law 16c` on the gameAsset publish step alone (`CLAUDE.md` §10). Confidence as merged: high on 9 rows, medium-high on 1, medium on 12, low-medium on 3.
