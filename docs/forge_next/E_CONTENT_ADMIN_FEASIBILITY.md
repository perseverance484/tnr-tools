...
1. **Source of the rows.** The per-class rows are the merged output of seven independent readers over the game source, committed as `evidence/admin-feasibility.json` (35 rows: 25 content classes CA-01 to CA-25, 10 cross-cutting CC-01 to CC-10). **Verification has since run and its verdicts are folded into that file**, one `verification` block per row carrying the verdict, the reasoning and, where a correction replaced a cell, the original row. It covered 33 of the 35 rows and corrected every row it touched; CA-23 is the only row whose zero-game-change answer changed, and this section was corrected to match. CC-08 and CC-10 were not reached before the session limit and are labelled not verified in the evidence file.
...
4. **CA row verdict tally, after verification** (25 content-class rows, counted from `evidence/admin-feasibility.json`): zero-game-change `YES` on 22 rows and `PARTIAL` on 3 (CA-12 sectorMap, CA-21 jutsu reskin moderation, and CA-23 content backups, which its verifier moved to `PARTIAL` because triggering a snapshot is limited to the four backup-control roles). CA-04 gameAsset moved the other way: its verifier found the bytes verdict misleading and rated it `YES` for record work at client parity, while the unhide step stays forbidden by engine law 16c. Four of the 22 are feasible but do not belong in a Content Admin surface on ownership grounds rather than capability grounds: CA-25 balance knobs, CA-17 ranked-season rewards, CA-18 activity-streak rewards and CA-19 raid damage thresholds, all of them reward or balance values the director owns (`CLAUDE.md` section 10; see K-53). One further step is feasible in the procedures and forbidden by repository law: the gameAsset unhide, which engine law 16c rules must not be built (`docs/ENGINE_LAWS.md:46`). Adversarial verification covered 33 of the 35 rows and corrected every one it touched, almost always in detail rather than in verdict: CA-23 is the single row whose zero-game-change answer changed. The two rows it did not reach, CC-08 and CC-10, are labelled not verified in the evidence file; CC-08's substance is independently established in `H_RISK_REGISTER.md` R-31, which was verified against the pin by the planning owner.
...
**Hazard.** Forge maps the tRPC code `UNAUTHORIZED` to the class `SESSION` (`forge/src/transport/outcome.mjs:61`) and `AuthState.refuse()` moves the whole session to SIGNED_OUT (`forge/src/transport/auth.mjs:199-201`). So the two families that **do** carry a machine-readable code are exactly the ones Forge would mis-report as a lost session. A role pre-check is required before `ai.getAiProfile` or any `skillTree.*` path is ever adopted (CC-01, R-26).
...
The limiter is a 60-request sliding window per 60 seconds, keyed per procedure path per user (`app/src/server/api/trpc.ts:123`, pin), and it is attached to `publicProcedure` by default (`trpc.ts:211-213`, pin) but is composable: `item.ts:1523` and several `combat.ts` procedures compose it onto protected procedures, so "protected" does not by itself mean unlimited (`trpc.ts:211-213`, pin). A trip is a **penalty, not a retry signal**: `movedTooFastCount` increments and both money and bank are multiplied by 0.99 before the request is rejected (`trpc.ts:166-168,179`, pin).
...
- **The protected reads this queue needs carry no limiter. That is a per-procedure fact, not a property of the auth class: Forge's own registry transcribes `limited` per procedure and never infers it from the base builder (`forge/src/transport/procedures.mjs:8-12`, which cites `item.splitStack` as the proof).** `profile.getAi`, `ai.getAiProfile` and the staff reads on maps, streak configs and raid thresholds carry no limiter at all. Where a protected equivalent exists, the admin surface should prefer it.
