...
The brief reserves eleven decisions in its section 14; they are carried here as K-01 to K-05 and K-07 to K-12, each keeping the brief's wording. K-06 (capture data classification and what may persist to the public repository) is not one of the eleven: it was surfaced by the deep dive and is numbered in that run for continuity. K-18 and K-19 are not used: the numbering jumped when the register was split between the brief's reserved set and the questions the evidence raised, and the gap is left rather than renumbering entries other sections already cite. K-13 onward were surfaced by the deep dive and the design panel. Section K.0 records the questions the director ruled during the pass, so that this register never presents them as open again; the entries from K-20 onward record the questions the director explicitly left open after those rulings.
