...
The architecture recommendation (section F) and the roadmap (section G) take these three rulings as fixed inputs.

**One reading that is advisory, not ruled.** `RUL-2026-09-12-002` settles that Forge is the translation, orchestration and presentation layer over repository authority. Reading that against the expansion study's D1 to mean "Forge is the primary human content-development workspace, and project and authoring workflows are product scope rather than helper utilities around the runner" is the planning owner's inference, not a sentence the ruling contains. It is recommended, and the package is built on it, and the director should confirm or reject it rather than find it presented as settled. What follows from it either way is K-28 (how much repository-backed project state Forge may edit) and K-29 (whether the guide and infographic lanes become official). The Quest Studio foundation slice on `chatgpt/forge-quest-studio-foundation` is an active ChatGPT-owned Lane A implementation; under the one-writer rule this package does not patch it and the roadmap plans around its independent review rather than re-planning the same seam.

## K.1 Ownership classes

Every entry below carries a **Class** field, and the three index tables that follow are split on it. The split exists because `docs/workflows/DIRECTOR_DECISIONS.md` §1 reserves specific classes to the user and then says plainly: "Do not escalate ordinary engineering details that are already settled by source/contracts/tests." A register that presents an internal mechanism as a director ruling costs the director attention and stalls implementation for no product or safety reason, and it blurs the line between user-owned canon and engineering design.

- **Director decision.** The choice falls in a class §1 reserves: balance and tuning, rewards and rarity, publishing and hidden-to-live timing, final content direction or acceptance, final player-facing UX where several valid experiences exist, art direction, and whether a risky production workflow should change. `docs/DEVELOPMENT_WORKFLOW.md` §5 adds live-game actions to the same list. Fable may recommend; Fable may not settle it by implementation order. Ruled entries are recorded in `docs/RULINGS.md` and the operative rule moves to its canonical owner (`DIRECTOR_DECISIONS.md` §4).
- **Engineering decision (Fable).** The choice is constrained by source, contracts, tests and the director-approved product boundary, and Fable owns it as implementation owner (`CLAUDE.md` §1; `docs/DEVELOPMENT_WORKFLOW.md:27`). Each such entry states **Fable's position** now, so the register records an answer rather than a question; the position is decided for real in the implementation brief for the phase named in the entry, defended there against the evidence the entry collects, and verified by independent review (`CLAUDE.md` §12). It is recorded in the governing plan, brief or handoff, not in `docs/RULINGS.md` (`DIRECTOR_DECISIONS.md` §4). A position is not a ruling: a reviewer may reject it, and nothing here licenses implementation, which still needs a director handoff and a committed brief (`CLAUDE.md` §11).
- **Split.** The row contains both. The **director half** is named and waits; the **engineering half** is named and does not. The common shape is a credential, exposure or authority question the director owns, wrapped around a mechanism Fable owns once the ruled constraint is fixed.

**Four classes this register treats as reserved that §1 does not name, stated as an extension rather than smuggled in.** §1's list ends with "any other class explicitly reserved by `docs/DOCTRINE.md` or a task contract", so an addition is legitimate only if something reserves it. For these four, nothing in the repository does, and the honest thing is to say so and let the director confirm or collapse each:

| Extended class | Entries that rest on it | Why this package treats it as the director's | If the director rejects the extension |
|---|---|---|---|
| **Operator-facing UX**, where §1 says *player*-facing | K-53, K-55, K-58, K-59, and K-34's director half | Forge has no player surface at all, so reading §1 literally would leave its entire UX unreserved, which cannot be what §1 means for an operator tool | those entries become engineering, decided in the phase brief |
| **Credential possession and scope** | K-15 and K-26 director halves | `CLAUDE.md` §5 governs how credentials are handled and §6 makes repository access not live-game authorization, but neither reserves the *decision*; the package treats a token's blast radius as too consequential for an implementer to settle alone | Fable picks the narrowest scope that works and defends it in the brief |
| **What may be exposed or persisted publicly** | K-06 and K-17 director halves | the repository is public and the data is players'; `DOCTRINE.md` does not name the class, and K-06 is not one of the brief's reserved eleven | the tier taxonomy becomes an engineering choice constrained by the existing allowlist |
| **Lane assignment** | K-61 as originally written; now none | the director did assign the Studio slice explicitly, so the practice exists; the class is not written down anywhere | already collapsed — K-61 is engineering, resolved by the default role model |

The extension is conservative in one direction and not the other: it hands the director **more** than §1 strictly requires. An independent reviewer should test it from both sides — whether a class was added without warrant, and whether an entry was retained under an added class when it did not need to be.

Where an engineering entry has a genuine user-owned consequence hiding inside it, the entry names that consequence explicitly under **Director-owned consequence** rather than escalating the whole mechanism. Most of them have none.

Nothing in this classification changes an id. Other sections cite these entries by number and the numbers are stable; only the ownership statement is new.

## K.2 Director decisions (awaiting a ruling)

Thirty-nine entries, plus the two unused numbering gaps kept for continuity. K-01 is listed because it was ruled mid-pass and its remainder is a director decision; K.0 above records what it settles.
...
## K.3 Split decisions (a director half and an engineering half)

Five entries. The director half of each is the only part that waits.

| ID | Decision | Blocks |
|---|---|---|
| K-06 | **Director half:** what classes of captured data may persist to the public repository. Engineering: the tier mechanism | Phase 1 |
| K-15 | **Director half:** whether the admin's device holds a repository credential at all, and its scope. Engineering: the sync or relay mechanism | Phase 3 |
| K-17 | **Director half:** which non-content procedures Forge may read at all. Engineering: the per-row audit and its re-check gate | Phase 1 |
| K-26 | **Director half:** the credential scope the operator's device must hold. Engineering: the trigger model (dispatch, source-push or hybrid) and the branch mechanics under it | Studio phase; K-15 |
| K-34 | **Director half:** whether the product carries authorization vocabulary at all. Engineering: how a denial is detected and anchored to the pin | Phase 4 (admin) |

## K.4 Engineering decisions (Fable-owned, not awaiting a ruling)

Fifteen entries. Each carries Fable's position and the phase brief that fixes it. They appear here so the evidence is not lost and so a reviewer can check the position against it, not because the director must answer them. The third column names the brief that fixes each one, not a phase it blocks: none of these blocks a freeze.

| ID | Decision | Fixed in the brief for |
|---|---|---|
| K-13 | Refresh the game-source pin before Forge Next implementation | Phase 0 |
...
| K-60 | Integrating the release-loader test fix that also exists on the ChatGPT branch | retirement gate G-09; named in the phase-0 integration plan, blocking no phase |
| K-61 | Ownership of `forge/src/studio/*` for the design-system absorption once the seam integrates | nothing; the role model already answers it |
...
**Class:** director decision. The choice belongs to a class `docs/workflows/DIRECTOR_DECISIONS.md` §1 reserves to the user; Fable may recommend, and may not settle it by implementation order.
...
**Class:** split. **Director half:** what classes of captured data may persist to the public repository at all. **Engineering half:** how the tiers are declared, enforced at the persist boundary, exported and shown in the capture library. Only the director half waits on a ruling; the engineering half is Fable's to decide and defend in the implementation brief, and is verified by independent review.
...
**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): a provenance-refresh pass inside phase 0, with the drift tools re-run at every phase gate and the 45x re-extraction kept as its own reviewed pass through the `schema_diff` adoption gate. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none. The measured evidence is that the derived contracts regenerate byte-identically, so this changes no contract and no content.
...
**Why that position:** (a). It is cheap, it removes an 81-commit explanation from every review, and it is not a contract change; the 45x adoption pass is the one that needs care.
...
**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** keep the bundle unminified until the size-budget gate exists, then adopt minification only when the CI rebuild-parity job is the accepted reviewer proof. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none. Reviewability of a checked-in artifact is an engineering and independent-review concern.
...
**Why that position:** (c), in phase 0 for minification only after the reviewer confirms the rebuild gate is the accepted proof.
**Can defer:** yes.

### K-15 GitHub credential model on the admin's device

**Class:** split. **Director half:** whether the admin's device holds a repository credential at all, and its maximum scope. **Engineering half:** the sync or relay mechanism that satisfies whichever scope is ruled. Only the director half waits on a ruling; the engineering half is Fable's to decide and defend in the implementation brief, and is verified by independent review.
**Why it matters:** results sync and approval-state writes need a fine-grained PAT stored in the browser (`tnr_bk_gh_v1`); the admin may not have or want one; `RUL-2026-09-08-001` is adjacent but not on point: its subject is how the agents authenticate to `tnr-tools` (ChatGPT uses the repository-scoped connector rather than a pasted token), and its rationale is that long-lived pasted credentials add exposure when repository-scoped authentication is available. It does not rule on a human operator's own browser-stored token, which is why this decision exists rather than being read off that ruling.
...
**Class:** split. **Director half:** which non-content procedures Forge may read at all, because that is an exposure question. **Engineering half:** the per-row audit (kind, auth class, limiter, default tier) and the gate that re-checks it when the pin moves. Only the director half waits on a ruling; the engineering half is Fable's to decide and defend in the implementation brief, and is verified by independent review.
...
- More behaviour: a dialog sheet listing secondary destinations is the package proposal (`D_VISUAL_SYSTEM.md`);
- badge and attention treatment: static counts and dots, never animated (MOB:365; `D_VISUAL_SYSTEM.md`);
- whether active work or recovery alters the shell: the package recommends keeping the bottom bar visible during a run with the top-bar underline as the run signal (MOB:367; `D_VISUAL_SYSTEM.md`).
...
**Consequence:** any choice must satisfy COL:43-48 and MOB §11. The package's RECOVERY on the caution hue must be argued against COL's indigo/violet (`D_VISUAL_SYSTEM.md` gives the argument). The Studio's compile colour must move once the mapping is fixed (RM-03 absorption).
...
**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (b) for Mission now and option (c) as the source for the Story family when K-27 reaches it, with decision slots first; under every option `meta` leaves the hash or the hash is documented as covering it. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none directly; the subtype order it serves is K-27, which stays the director's.
...
**Why that position:** (b) for Mission now, (c) as the source for the Story family when K-27 reaches it; decision slots first, because they gate the decision cards in QS §10 (QS:356-362). Reasoning: it changes nothing the seam already proves and adds the one section that removes a relay.
...
**Class:** split. **Director half:** the credential scope the operator's device must hold, under least privilege. **Engineering half:** the trigger model (dispatch, source-push or hybrid), the branch mechanics and the run correlation, once the ruled scope and the UX bar are satisfied. Only the director half waits on a ruling; the engineering half is Fable's to decide and defend in the implementation brief, and is verified by independent review.
**Why it matters:** the foundation slice implemented the trigger model that the Tier A design did not recommend first, and that choice widens the credential the operator's phone must hold. The implementation is not the ruling. What the director rules is the scope that credential may have; which trigger satisfies that scope is then Fable's to choose and defend, and "ratify or revert" is the shape the answer takes once the scope is fixed.
...
**Recommendation (advisory):** rule the scope before any second adapter and before the first phone rehearsal; Fable's audit of the credential model, which QS:340 assigns to Fable, is written into section F before the ruling is requested. No option is recommended here, because each of the three carries a different credential scope and that scope is the half the user owns (`CLAUDE.md` §10). Once it is ruled, Fable takes the narrowest trigger the ruled scope allows — (b) under a Contents-only scope, (a) if the wider Actions scope is granted, (c) only if the ruling explicitly permits both — and defends that choice in the phase-S brief.
**Can defer:** no.
...
### K-34 Distinguish authorization (role) denial from other refusals: OPEN, split (the source audit is done)

**Class:** split. **Director half:** whether the product carries authorization vocabulary at all (a visible NOT AUTHORIZED lamp or per-action denial label). **Engineering half:** how a role denial is detected and anchored to the pinned source, and the test that fails on an upstream rewording. Only the director half waits on a ruling; the engineering half is Fable's to decide and defend in the implementation brief, and is verified by independent review.
...
**Evidence:** the auth model has four states and none for role denial (`forge/src/transport/auth.mjs:54`, Source-verified); `classifyError` has no FORBIDDEN class (`forge/src/transport/outcome.mjs:58-73`); a mutation refusal arrives as HTTP 200 `success:false` and becomes a FAILED item (`forge/src/transport/outcome.mjs:39`; `forge/src/runner/runner.mjs:640`); role gates are resolver-level (`H_RISK_REGISTER.md` R-08). The safety contract requires the distinction only where the procedure surfaces it (SSC:83, 293-295) and the copy contract gives the wording for that case (CPY:386-396). The package's shell proposes the NOT AUTHORIZED lamp (`D_VISUAL_SYSTEM.md`,504`). The design index forbids inventing backend capability to satisfy a mockup (IDX:145). Reconciliation conflict CF-04 routes this question here.
**Options:** (a) adopt a per-action denial label for the thirteen role-only paths E.3 lists, generated as a table anchored to the pin and re-checked whenever the pin moves, so an upstream rewording fails a test instead of silently downgrading the label; (b) render every refusal with the server's own message under disclosure and add no authorization vocabulary at all; (c) adopt the pre-emptive role pre-check only, disabling what the role cannot do and saying why, with no label on the refusal itself.
...
### K-35 ABORTED: introduce a producer or drop it from the human vocabulary: OPEN (Fable)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (b): keep ABORTED unreachable and omit it from the label dictionary for v1. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** only if a user-facing abort control is later proposed; the control and its wording would then be a director UX decision.
...
**Why that position:** (b) for v1; revisit with a user-abort feature. Reasoning: no scenario in the current inventory needs an abort that pause does not cover.
**Can defer:** yes.
**Blocks phase:** none.

### K-36 Canonical enforcement of the mission profile shape in `mission.py`: OPEN (Fable; a canonical-owner change with one named director consequence)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): enforce the profile shape in `mission.py`, its canonical owner, under its own Lane A brief, with a profile-declared strict flag defaulting on for new ranks and the selftest fixture repaired. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** yes, and it is named: if enforcement would refuse content the director has already accepted, the refusal list goes to the director before the flag is turned on for existing ranks (content acceptance, `CLAUDE.md` §10).
...
**Why that position:** (a) with a profile-declared strict flag defaulting on for new ranks; Fable drafts the brief, the director rules. Reasoning: enforcement in the owner is the only way the browser and the compiler agree by construction.
**Can defer:** until the Studio promotion contract; not beyond the second adapter.
**Blocks phase:** Studio phase.

### K-37 `studio/*` branch namespace, retention, cleanup and conflict handling: OPEN (Fable)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (b) plus a nightly TTL sweep for abandoned branches and a compiler/profile revision drift warning in the envelope. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none. Declaring the namespace in `docs/DEVELOPMENT_WORKFLOW.md` and routing the compiler in `docs/00_INDEX.md` are canonical-owner changes with their own Lane A brief and review, not director rulings.
...
**Why that position:** (b) plus a nightly TTL sweep for abandoned branches, and a compiler/profile revision drift warning in the envelope. Reasoning: the branch is a request record; its lifetime should follow the request's.
**Can defer:** until after the first rehearsal.
**Blocks phase:** Studio phase (before broad use).

### K-38 Worker refusal and cancellation observability: OPEN (Fable; the director only if the credential widens)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): persist a refusal envelope before every exit path, plus poll backoff and a check-later state, which needs no new credential. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** only through K-26: option (b) would widen the operator's credential, so it may not be taken unless K-26's director half allows that scope.
...
**Why that position:** (a) plus backoff; (b) only if K-26 ratifies dispatch. Reasoning: the envelope is already the observation channel and needs no new credential.
**Can defer:** no; the current state violates SSC:20 and RB:280.
**Blocks phase:** Studio phase.

### K-39 Artifact promotion gate and the journaled Studio to runner contract: OPEN (Fable)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): promote to `push/` through `pushpack.py` at an explicit package gate, keeping one runner ingress, with the manifest hash added to the envelope and verified at import. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none. It adds no authority: the live start stays the operator's explicit action and no Start control enters the Studio.
...
**Why that position:** (a) for v1 with the manifest hash verified at import. Reasoning: it is the smallest change to a reviewed ingress.
...
**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): the Studio writes `project` into the source and the roadmap gains a resource pointer, each side recording only what it owns. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** only through K-28, which rules how much repository-backed project state Forge may edit at all.
...
**Why that position:** (a). Reasoning: each side records only what it owns.
...
### K-47 Review annotations store: OPEN (Fable; follows K-09, which is the director's)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): co-locate annotations with the review file K-09 places in the repository, one file per package. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** K-09 owns where approval state lives; once it is ruled, the annotation store follows it rather than choosing for it.
...
**Why that position:** (a). Reasoning: one file, one owner, one diff.
**Can defer:** yes.
**Blocks phase:** Phase 4.

### K-48 Reuse search placement: OPEN (Fable)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (b): a shared Research Library with deep links, with only the zero-live catalog and committed-capture search embedded in the Studio. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none. What may be read and what may persist are K-17 and K-06.
...
**Why that position:** (b), with zero-live catalog and committed-capture search embedded. Reasoning: only reads that cost nothing belong inside the editor.
**Can defer:** yes.
...
**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (b): a shared graph compiler with policy adapters, which is the model three subtypes already share in the design. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none directly; the rollout order it serves is K-27, which stays the director's.
...
**Why that position:** (b). Reasoning: three subtypes share one graph model in the design.
...
### K-52 How the admin surface learns the signed-in account's role: OPEN (Fable; one named cost consequence)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** adopt option (a): one public `profile.getPublicUser` per session, cached, budgeted and shown in the readiness card. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** one, and it is small: the call spends a budget unit, and the limiter's money and bank penalty applies only on a trip, which the margin-0.5 budget exists to prevent. If the director wants zero budget spend on an admin screen, that constraint changes the answer to (b).
...
**Why that position:** (a), cached for the session and shown as a readiness lamp, because the cost is one request and the payload is the smallest that answers the question. The choice is the director's because it trades a budget unit against reading more of the account record.
...
### K-60 Integrating the release-loader test fix under one-writer discipline: OPEN (Fable; no director ruling needed)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** Fable authors a minimal fix that relaxes only the marker assertion, on a Fable branch, as ordinary engineering, and the phase-0 brief records that a second fix exists on the ChatGPT branch. If the Studio integration lands first, the fix arrives with it and the Fable change becomes a no-op. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** none. Nothing player- or operator-facing changes; a red baseline is a gate condition, not a product choice.
**Why it matters:** `main` is red by one test after every release pin, and a fix for it also lives on a ChatGPT-owned branch bundled with unrelated assertion changes. Gate G-09 ("gates green on `main`") cannot close until one of them lands. The first draft of this entry treated that as a director assignment; independent review corrected the reading and it is corrected here. The one-writer rule governs **a branch**, not a file: `docs/DEVELOPMENT_WORKFLOW.md:112` reserves an active Lane A branch to a single writer, and two independently owned branches may both contain a change to `release_loader.test.mjs`. The overlap is resolved where overlaps are always resolved, at integration and review, and it is an ordinary engineering choice rather than a product or safety one.
**Evidence:** the red baseline and its cause are in `A_ARCHITECTURE_MAP.md` §A.10 and `H_RISK_REGISTER.md` R-11; the coupling of the fix to unrelated changes on the feature branch is R-22; the one-writer rule and its scope are `CLAUDE.md` §4 and `docs/DEVELOPMENT_WORKFLOW.md:112`, which reserve an active Lane A branch to a single writer and say nothing about a file appearing on two branches.
...
**Consequence:** (a) couples an unrelated schedule to a one-line test fix; (b) means two branches contain a change to the same test file, which the integration resolves and which one-writer discipline does not forbid, since that rule reserves a branch to one writer rather than a file to one branch; (c) means every branch starts red and the first green commit is noise, which is R-11 as written.
**Why that position:** (b) is one line, it is reviewable on its own, and it does not couple a green baseline to an unrelated integration schedule. It is written so that (a) supersedes it harmlessly: if the Studio integration lands first, its relaxation is already in `main` and the Fable change is a no-op or a trivial conflict resolved in favour of the integrated version. What the phase-0 brief owes a reviewer is the statement that both exist and which one landed, not a ruling.
**Can defer:** it is not deferred, it is done in phase 0's own work; no phase waits on a person to answer it.
**Blocks phase:** none. Retirement gate G-09 needs the fix landed, and phase 0's integration plan names it.

### K-61 Ownership of the Studio files for the design-system absorption: OPEN (Fable by default; the director acts only to assign otherwise)

**Class:** engineering decision (Fable). `docs/workflows/DIRECTOR_DECISIONS.md` §1 says not to escalate ordinary engineering details that source, contracts and tests already settle, so this row does not wait on a ruling. **Fable's position:** the existing role model already answers it: Fable is the normal implementation owner (`CLAUDE.md` §1; `docs/DEVELOPMENT_WORKFLOW.md:27`) and the Quest Studio slice was an explicit slice assignment, not a permanent transfer of Forge ownership. After the frozen slice is independently reviewed and integrated, `forge/src/studio/*` returns to Fable for the absorption, which is option (a). Option (c) is ruled out by the approved visual direction. It is decided in the implementation brief for the phase named below, defended there against the evidence in this entry, and verified by independent review. **Director-owned consequence:** only if the director wants to assign another ChatGPT slice instead, which is an assignment they make rather than a question this register holds open.
**Why it matters:** the Quest Studio foundation ships a second full-screen shell with its own palette, its own confirmation and a mount outside the composition root. The approved direction is one shell and one token set, so the absorption is real work on files that a ChatGPT branch owns until it integrates. The first draft asked the director who owns them afterwards; independent review pointed out that the role model already answers it, and it does: Fable is the normal implementation owner (`CLAUDE.md` §1; `docs/DEVELOPMENT_WORKFLOW.md:27`) and the Studio slice was an explicit slice assignment, not a permanent transfer of Forge ownership.
...
**Why that position:** (a) is what the default role model produces once the slice's own review closes, and it gives the design system one owner, which is the condition the approved direction needs. (c) is ruled out by that direction, not by preference. (b) remains available at any time, but it is the director assigning a further ChatGPT slice rather than this register holding a question open; the absorption brief simply names whichever owner holds at the time it is written.
**Can defer:** nothing waits on it. The absorption cannot start before the slice is reviewed and integrated in any case.
**Blocks phase:** none. The Studio phase's shell work names its owner in its brief, as every phase brief does.
