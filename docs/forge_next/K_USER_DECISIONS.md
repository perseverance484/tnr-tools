...
- **Director decision.** The choice falls in a class §1 reserves: balance and tuning, rewards and rarity, publishing and hidden-to-live timing, final content direction or acceptance, final player- or operator-facing UX where several valid experiences exist, art direction, whether a risky production workflow should change, credential possession and scope, what may be exposed or persisted publicly, and lane assignment. Fable may recommend; Fable may not settle it by implementation order. Ruled entries are recorded in `docs/RULINGS.md` and the operative rule moves to its canonical owner (`DIRECTOR_DECISIONS.md` §4).
