...
- `wf01` to `wf17` are the screens, ending with the degraded and refused states (session not ready, an action the role cannot perform, the rate limiter tripped, a read whose body did not persist, storage the browser would not make durable); `review/` holds rendered captures of six of them (JPEG, 1400 px wide) for reviewers without a browser.
