...
- **session_lamp**:
  ...
  - **NOT_AUTHORIZED (conditional)**:
    ...
    - **word**: NOT AUTHORIZED
    - **contrast**: 6.94
    - **status**: CONDITIONAL, not a fifth state today. Rendered only if the section E content-admin feasibility audit finds a source-verified role-denial signal in the admin procedures at the pin. Today auth.mjs:54 has four states (UNKNOWN, PROBING, READY, SIGNED_OUT); classifyError (outcome.mjs:58-73) has no authorization class; a role refusal arrives as HTTP 200 success:false and becomes a FAILED item, indistinguishable from guards and name collisions (H_RISK_REGISTER.md:16, R-08). SSC:83 and SSC:291-295 ask for the distinction only 'when surfaced by the underlying procedure'; IDX:145 forbids inventing the capability. If a stable signal exists, a classifyError class and an AUTH extension are a Lane A deliverable below the UI seam (H_RISK_REGISTER.md:21, R-13) and the wording is CPY:390-394; this is K-34 (proposed) in D2.10. If no signal exists this row is dropped and the lamp keeps four states ('not distinguishable from source'). Source-verified for the facts; the lamp itself is a proposal.
    - **note**: construction if rendered: hollow --warn lamp plus a --warn-fill banner naming the action; never 'sign in again' (SSC:512). READ FAILED is never on the lamp: it is shown on the item row in --bad.
...
The rows above cover 15 of the 25 WCM responsibilities under this file's names (mapping column). The remaining ten are added here so that no WCM responsibility is unowned; the 'forge data source today' column names the state Forge already holds, so that no row implies a capability the source lacks (WCM:607-629; H_RISK_REGISTER.md:25, R-16). Responsibility text is the owner's; only the construction is this file's.
...
| **PublishControl** | WCM:120-122; WCM:615-617 | none today: no publish or unhide recipe exists (A_ARCHITECTURE_MAP.md:151) | HOLD | the publish-primary ink construction (ActionButton row) is reserved; the control is never rendered enabled from visual readiness alone; a placeholder until the section E feasibility audit and K-03 to K-12. |
...
- **auth**: four lamp states with distinct glyph + word (READY ✓, CHECKING …, UNCONFIRMED ?, SIGNED OUT ✕ filled; auth.mjs:54) plus NOT AUTHORIZED ⊘ hollow amber only under the D2.2 session_lamp condition (K-34); READ FAILED lives on the item, never on the lamp.
...
- S4 (admin, after K-03/K-05/K-12): content pills, DiffView, PreviewFrame, the gold-outline admin rows, the publish seal + PUBLISH band + armed sheet, NOT AUTHORIZED handling only if K-34 finds a source signal.
...
- G-A5 NOT AUTHORIZED session state (conditional on a source signal, D2.2 session_lamp; K-34): hollow --warn lamp + --warn-fill banner naming the action; READ FAILED on the item only (from A/B via G1/G13)
