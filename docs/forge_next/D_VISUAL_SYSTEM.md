...
- **DONE**:
  ...
  - **word**: DONE
  - **contrast**: 15.12
  - **note**: neutral state pill; the OUTCOME lamp beside it (VERIFIED ALL --ok filled / UNVERIFIED --warn outline / FAILED --bad outline, from jobOutcome) carries the verdict (judge G9; A_ARCHITECTURE_MAP.md:107; app.mjs:384 exports both). DONE is never green on its own; today styles.mjs:44 paints it green.
- **ABORTED**:
  - **text**: #ff8a80
...
- **session_lamp**:
  ...
  - **NOT_AUTHORIZED**:
    ...
    - **word**: NOT AUTHORIZED
    - **contrast**: 6.94
    - **note**: NEW (judges G1/G13; brief section 10; amendment section 9): authenticated but the role a Content Admin action needs is missing; hollow --warn lamp plus a --warn-fill banner naming the action; never 'sign in again'. READ FAILED is never on the lamp: it is shown on the item row in --bad.
...
- **touch**: 44 px minimum everywhere; 48 px risk buttons; 56 px list rows and bottom-nav items; pills are not tap targets (rows are).
...
- **kind**: text; **pair**: warn on paper (PAUSED/INCOMPLETE pill, NOT AUTHORIZED lamp); **fg**: #f08a3c; **bg**: #121b26; **ratio**: 6.94; **passes**: True
...
| **Banner / Callout** | full-width, 2 px hard border in its construction, glyph + bold lead sentence (15/16 px) + secondary sentence (13 px) + next safe action; constructions: info (--acc-fill, --acc lead), ok (--ok-fill), warn (--warn-fill), bad (--bad-fill), SENT (hatched --sent-fill, dashed --sent border, '? 1 WRITE UNCONFIRMED · nothing is retried; Reconcile reads the server first'), NOT AUTHORIZED (--warn-fill, ⊘, names the action the role lacks), full-capture advisory (▣ FULL in --warn + the repository-persistence sentence of screens.mjs:123-125) | preflight; run; jobs; admin queue; settings | static; role=alert when it appears; never auto-dismisses | toasts are a secondary echo: ok/info auto-dismiss 4 s, warn/bad persist until tapped and stack (judges G5/G7); a toast is never the sole carrier of PAUSED/ORPHANED/SENT |
...
- the session lamp and the auth banner; the NOT AUTHORIZED banner
...
- **auth**: five lamp states with distinct glyph + word (READY ✓, CHECKING …, UNCONFIRMED ?, SIGNED OUT ✕ filled, NOT AUTHORIZED ⊘ hollow amber); READ FAILED lives on the item, never on the lamp.
...
- **16. Style board generic 'Something Went Wrong → Try Again' error state** Evidence: safety contract section 16: no generic Retry on an ambiguous write; A_ARCHITECTURE_MAP.md:141 (SENT → PLANNED is structurally impossible) Smallest correction: 'Try again' exists only for retry-safe classes (a GitHub listing, a cache open); mutation failures render the halt/orphan card Decision: none (non-negotiable)
- **17. Style board five-tile mode SELECTOR** Evidence: reconciliation matrix: mode must be derived from the plan/journal/action, never chosen; choosing a colour must not look like acquiring authority Smallest correction: the band is display-only; there is no mode selector Decision: none

## D2.6 Implementation sequence for the visual shell

- S1 (visual only, class names preserved): tokens on .f-host/.f-app, typography, pill() -> STATE table with glyph box, ModeBand, session lamp with the five states, banners by construction, the four button lanes, disabled-without-opacity, prefers-reduced-motion + prefers-contrast + Daylight blocks, the contrast/allow-list contract test. Runs on the existing five screens; forge/test/ui.test.mjs selectors keep working.
...
- S4 (admin, after K-03/K-05/K-12): content pills, DiffView, PreviewFrame, the gold-outline admin rows, the publish seal + PUBLISH band + armed sheet, NOT AUTHORIZED handling.
...
~16-20 KB unminified CSS (~5 KB gzip) in one sheet: ~75 tokens, ~160 rules, 3 media queries (768, 1024/1280, reduced-motion) + prefers-contrast, 1 keyframes (sheen), 4 hatch gradients, ~12 elevation/glow declarations; ~3 KB of createElementNS atmosphere/lane glyphs; ~350 lines of JS for pill/band/lamp/progress/sheet/shell. Total UI delta ~+18 KB on a 398 KB bundle (A_ARCHITECTURE_MAP.md:171), under 5 %; no dependency; no asset.
...
- G-A5 NOT AUTHORIZED session state: hollow --warn lamp + --warn-fill banner naming the action; READ FAILED on the item only (from A/B via G1/G13)
...
## D2.10 Decisions this section surfaces (mirrored in K)
...
## D2.11 Evidence cited by the synthesis
...
- docs/forge_next/A_ARCHITECTURE_MAP.md:93-107 (job/item states, transitions, outcome separate from state), :141 (SENT -> PLANNED impossible), :150-152 (writes behind confirm; no publish action today; full-persist allowlist), :171-177 (bundle 397,984 B; ui 59,501 B)
- docs/forge_next/K_USER_DECISIONS.md:9-15 (K.0 rulings), :19-46 (K-01, K-02, K-12, K-20..K-24, K-30), :50-56 (K-01 ruled; A and B kept as graft sources only)
- docs/forge_next/H_RISK_REGISTER.md:14 (R-06 mobile performance bans embedded art), :25 (R-16 mockup-implied capabilities; status blocks never poll)
...
- scratchpad/consolidated/docs/design/FORGE_NEXT_UI_DESIGN_INPUT_INDEX.md:14-26 (Tier A: visual direction and colour-semantics separation rule), :46-48 (v0.1 is a proposal, not a lock), :125-141 (mode taxonomy, palette, typography deliberately unsettled), :143-147 (conflict handling: later ruling wins; never invent capability to satisfy a mockup)
- scratchpad/consolidated/docs/design/FORGE_NEXT_COLOR_SEMANTICS.md:159-163 (ruling: operation context vs semantic outcome must be distinct), :169-176 (mode families; label + icon required), :180-189 (reserved semantic families; violet for SENT only if distinct from PUBLISH), :191-198 (collision rules 1-6), :200-214 (reference values are not locks)
- scratchpad/consolidated/docs/design/FORGE_NEXT_DESIGN_SYSTEM_V0_1.md:32-78 (v0.1 token values shown side by side), :82-95 (modes), :125-139 (type scale), :161-173 (shape language), :175-201 (glow allowed/avoid), :203-225 (button hierarchy; production-action wording), :227-281 (component vocabulary), :283-297 (state groups), :355-382 (motion timing), :418-434 (unresolved)
- scratchpad/consolidated/docs/design/FORGE_NEXT_STYLE_BOARD_INVENTORY.md:39-66 (board palette), :69-84 (board semantic colours), :86-119 (five-mode taxonomy incl. Review; chronology flag), :123-139 (board type scale), :219-238 (Success/Warning buttons question), :270-282 (mode selector risk), :360-370 (generic Try Again constraint), :434-445 (transcript questions)
- scratchpad/consolidated/docs/design/FORGE_NEXT_DESIGN_RECONCILIATION_MATRIX.md:62-63 (Review mode and mode colour mapping: APPARENT CONFLICT), :66 (mode selector OPEN BY CONTRACT), :83 (typography BOARD ADDS DETAIL), :90 (Success/Warning buttons OPEN BY CONTRACT), :96-100 (SENT/CONFIRMED/INCOMPLETE/orphan/auth NEEDS VALIDATION), :136-149 (frozen until export), :164-178 (transcript questions)
- scratchpad/consolidated/docs/design/FORGE_NEXT_ICON_MOTION_SEMANTICS.md:18-44 (three layers never share a language), :46-103 (mode metaphors; avoid checkmark for write, refresh alone for recovery), :105-176 (state metaphors; ambiguity needs its own glyph), :189-211 (shape semantics), :240-296 (motion roles; stop promptly; SENT never a spinner), :324-333 (reduced-motion contract), :335-346 (no theatrical motion for danger), :370-376 (critical states never auto-dismiss)
- scratchpad/consolidated/docs/design/FORGE_NEXT_SAFETY_STATE_PRESENTATION_CONTRACT.md:22-105 (six axes), :107-185 (certainty hierarchy PLANNED..SKIPPED), :187-233 (job states; INCOMPLETE never green; DONE alone is not success), :264-295 (auth incl. authorization/role denial), :297-322 (capture persistence two questions), :336-386 (recovery action grammar), :388-402 (five-cue stack + sixth for ambiguity), :404-415 (colour rules), :417-470 (component requirements), :505-517 (anti-patterns)
