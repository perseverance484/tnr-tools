...
- S5 (Studio absorption, after seam integration and the K-61 ownership ruling): the Quest Studio shell moves onto this token set and ConfirmationSurface (D2.2 'Studio shell colours'); until then it keeps its own stylesheet and is not restyled from this branch (one writer per branch, CLAUDE.md section 4).
