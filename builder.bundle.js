// TNR content builder bundle v4.17 - loaded via @require by the tiny VM loader.
// v4.17 changes (three lint bugs; no data was at fault):
...
// v4.17 changes (bloodline entity + data-driven phase order):
...
try{const bundle={builder:'v4.17',at:now(),entries:rows.map(r=>({name:r.name,srcId:r.key||null,entity:r.entity,slot:r.slot,state:r.state,detail:r.detail||'',id:r.outId||idmap[r.key]||r.targetId||null,pushed:r.pushed||null})),idmap};dl('tnr_results_'+Date.now()+'.json',JSON.stringify(bundle,null,1))}catch(_e){}
