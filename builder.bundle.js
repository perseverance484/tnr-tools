// TNR content builder bundle v4.16 - loaded via @require by the tiny VM loader.
// v4.16 changes (bloodline entity + data-driven phase order):
...
try{const bundle={builder:'v4.16',at:now(),entries:rows.map(r=>({name:r.name,srcId:r.key||null,entity:r.entity,slot:r.slot,state:r.state,detail:r.detail||'',id:r.outId||idmap[r.key]||r.targetId||null,pushed:r.pushed||null})),idmap};dl('tnr_results_'+Date.now()+'.json',JSON.stringify(bundle,null,1))}catch(_e){}
