// TNR content builder bundle v4.14 - loaded via @require by the tiny VM loader.
...
const build=async sub=>{const L=sub||rows;if(!sub)jcat=null;$g.disabled=$r.disabled=1;if(!skipPF){const pb=preflight(L);if(pb){$s.textContent='❌ preflight: '+pb+' entr'+(pb>1?'ies':'y')+' invalid, tap red rows. "skipPreflight":true bypasses.';$g.disabled=$r.disabled=0;return}}const uwf=(w,tr)=>{$s.textContent='⏳ upload limited '+((w/1000)|0)+'s try'+tr};
...
try{const bundle={builder:'v4.13',at:now(),entries:rows.map(r=>({name:r.name,srcId:r.key||null,entity:r.entity,slot:r.slot,state:r.state,detail:r.detail||'',id:r.outId||idmap[r.key]||r.targetId||null,pushed:r.pushed||null})),idmap};dl('tnr_results_'+Date.now()+'.json',JSON.stringify(bundle,null,1))}catch(_e){}
