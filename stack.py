...
back to Brandon, a law cited by a number that moved is a wrong answer.
...
def check(stack, quiet=False, ids_detail=False):
    ...
    for c in cat_present:
        d = json.load(open(os.path.join(stack, c)))
...
if __name__ == "__main__":
    ...
    sys.exit(check(args[0] if args else ".",
                   "--quiet" in sys.argv, "--ids" in sys.argv))
