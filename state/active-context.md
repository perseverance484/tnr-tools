...
**In progress:** wave: Content: ARTWORK ONLY - prose passes (Old Ghost, Tenth Name) reassigned to coworker per dauntless 2026-08-30. Avatars 9/9 done. Pin art COMPLETE: Repair/Dig/Watch/Deliver/Listen accepted 2026-08-30 (Listen v1 accepted by dauntless over QC reject). WIRING phase: push/10 (ten-mission capture) staged; push/11 objective-image edits generate from its bundle; @img picks need the five pin_*.webp via Files picker (not Gallery). Gather stays imageless per ruling (spec pin_gate says imageless = no pin; flag if live shows a default); Gather uses default collect_item pin per dauntless 2026-08-30; Chase pending dauntless ruling on twelfth) - 5 live per asset-manager view 2026-08-30 (Travel/Search/Pick/Fight/Climb, opaque navy house style); then backgrounds, scene portraits (Named-speak ruling pending). push/9 STATIC capture demoted to wiring-time (URLs). push/6 held on Terr wedge.
...
**Next:** Content on go. First: load check - installed SKILL.md must route builds to packs (confirms reinstall took); then Faceless Stray QC when the generation lands, else Old Ghost prose. Ambient: watch first pinned release, relay ritual switch when convenient, memory regeneration review.
