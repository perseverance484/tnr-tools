...
**State in one line:** Portrait wave CLOSED end-to-end: nine speakers generated/QC'd/approved, cast sheet posted, assets live via zip pack (16c, picker-free), all ten missions wired to speakers (18) + blank blanket (19), every write read-back verified; publish gate law candidate cited (quests.ts ~757-772). Builder v4.31 shipped (unknown-entity guard + stale-idmap heal) - VM refresh still owed, bundles stamp v4.29. Cast rulings complete incl rank handoff (A-rank -> Commander). Lane ARTWORK ONLY; publish gate: content admin.
...
**In progress:** wave: Portrait wave CLOSED. Next wave: SCENE BACKGROUNDS (6 specced + finale pair 1 each) on the wave_workflow loop: thread init + reference, gens with QC, sheet, one zip pack, verify, close. Icons wave after.
...
**Next:** Open on backgrounds wave: cut thread init (SCENE_BACKGROUND 3:2, house style, reference attach) + first card from the six specced. Owed (dauntless): junk-jutsu deletions x9, VM refresh to v4.31, e16 pronoun sweep, publish flip decision rides content admin. Owed (Claude): law-94 authoring via law process. Parked: Terr relay, WO-08.
