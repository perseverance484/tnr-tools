...
**In progress:** wave: Ten missions COMPLETE: wired, verified, wedge closed (builder v4.30), 26 probes deleted, spent manifests archived. Lane: ARTWORK ONLY - portraits + backgrounds + listing icons per state/art_backlog.md; gates: publish gate (content admin); micro-rulings open (silent nodes g30/g36/x1, x22 speaker, handler pronouns).
...
**Next:** Content on go. First: load check - installed SKILL.md must route builds to packs (confirms reinstall took); then Faceless Stray QC when the generation lands, else cut next portrait prompt in queue (prose lane is coworker's). Ambient: watch first pinned release, relay ritual switch when convenient, memory regeneration review.
