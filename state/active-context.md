...
**In progress:** wave: Ten missions COMPLETE: wired, verified, wedge closed (builder v4.30), 26 probes deleted, spent manifests archived. Lane: ARTWORK ONLY. Firm nine portraits COMPLETE + cast sheet posted; push/16 staged as v4.25 zip PACK (no picker) awaiting dauntless Repo->Build + bundle verify. Remaining art per state/art_backlog.md; gates: publish gate (content admin); micro-rulings open (silent nodes g30/g36/x1, x22 speaker, handler pronouns).
...
**Next:** Build+harden the wave workflow per state/wave_workflow.md (thread init -> gens -> sheet -> one zip pack -> verify -> close). First: verify push/16 results bundle when it lands. Gated: handler pair (pronouns + exemplar reuse call), silent nodes g30/g36/x1, x22 speaker, backgrounds wave, icons wave.
